...
int right_dest(char*, char*, int, uint16_t, uint16_t);
...
void process_packets(char* ip, int port, char* buffer, int size) {
    ...
    if(!right_dest(dest_ip, ip, port, udp_header->dest, udp_header->source)) return;
...
int right_dest(char* dest_ip, char* ip, int port, uint16_t udp_dest, uint16_t udp_src) {
    return strcmp(dest_ip, ip) == 0 && (ntohs(udp_dest) == port || ntohs(udp_src) == port);
}
