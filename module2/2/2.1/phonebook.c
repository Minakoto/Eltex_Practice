...
char init_entry(Entry *entry, char *first, char *last, short* cur) {
    short rnd = rand()%SIZE;
    while(find(entry, rnd, *cur) != -1) {
        rnd = rand()%SIZE;
    }
    (entry + *cur)->id = rnd;
    char res = init_person(&(entry+*cur)->person, first, last);
    zero_address(&(entry + *cur)->adress);
    zero_job(&(entry + *cur)->job);
    zero_contacts(&(entry + *cur)->contacts);
    return res;
}
